...
public interface ClienteRepository extends CrudRepository<Cliente, Long> 
{
	...
	@Query(nativeQuery=true, value="SELECT MONTHNAME(fechaSOAT) AS mes, COUNT(id_cliente) AS num_clientes FROM cliente WHERE EXTRACT(MONTH  FROM fechaSOAT) BETWEEN '01' AND '12' AND EXTRACT(YEAR FROM fechaSOAT) = :anio GROUP BY(EXTRACT(MONTH  FROM fechaSOAT))")
